...
function InvoiceProcessor() {
  ...
  const processPayments = async () => {
    ...
    try {
      ...
      const invoicePaymentsCount = response.data.data.filter(p => p.invoiceNo !== 'UNUSED').length;
      const unusedCount = response.data.data.filter(p => p.isUnused).length;
      
      setSuccess(
        `Successfully processed ${invoicePaymentsCount} invoice payments${unusedCount > 0 ? ` and found ${unusedCount} unused transactions` : ''}`
      );
    } catch (err) {
  ...
  const downloadCSV = () => {
    ...
    const invoicePayments = processedPayments.filter(p => !p.isUnused && p.invoiceNo !== 'UNUSED');
    ...
    setSuccess(`Downloaded ${invoicePayments.length} processed invoice payments`);
  };

  const downloadUnusedCSV = () => {
    const unusedTransactions = processedPayments.filter(p => p.isUnused || p.invoiceNo === 'UNUSED');
    ...
    const formattedUnused = unusedTransactions.map(payment => ({
      ...
      'Customer': payment.customerName.toUpperCase(),
      'Transaction Amount': payment.transactionAmount || payment.amount,
    ...
    setSuccess(`Downloaded ${unusedTransactions.length} unused transactions`);
  };
  ...
  const totalPaidAmount = invoicePayments
    .filter(p => typeof p.amount === 'number')
    .reduce((sum, payment) => sum + payment.amount, 0);
  
  const totalUnusedAmount = unusedTransactions
    .reduce((sum, payment) => sum + (payment.transactionAmount || payment.amount || 0), 0);
  
  const invoiceTotals = new Map();
  invoicePayments.forEach(p => {
    const current = invoiceTotals.get(p.invoiceNo) || { 
      totalPaid: 0, 
    ...
    current.isFullyPaid = p.isFullyPaid || Math.abs(current.totalPaid - current.invoiceAmount) <= 1;
    
    invoiceTotals.set(p.invoiceNo, current);
  });
  
  const paidInvoices = Array.from(invoiceTotals.values()).filter(inv => inv.isFullyPaid).length;
  const partiallyPaidInvoices = Array.from(invoiceTotals.values()).filter(inv => !inv.isFullyPaid && inv.totalPaid > 0).length;
  const unpaidInvoices = Array.from(invoiceTotals.values()).filter(inv => inv.totalPaid === 0).length;
      ...
      <div className="card">
        ...
        <div style={{ 
          ...
          <ol style={{ marginLeft: '1.5rem', lineHeight: '1.8' }}>
            ...
            <li><strong>Sorts invoices by date (DESCENDING)</strong> - Newest/last invoice is paid FIRST</li>
            ...
            <li><strong>Allocates payments sequentially:</strong>
              <ul style={{ marginLeft: '1.5rem', marginTop: '0.5rem' }}>
                <li>Pays the NEWEST invoice first with all available funds</li>
                <li>If money remains after first invoice, pays the next newest invoice</li>
                <li>Continues until all invoices are paid or funds run out</li>
                <li>Shows EXACT amount paid (if invoice is 12,500 and fully paid, shows 12,500)</li>
              </ul>
            </li>
            <li><strong>Customer isolation:</strong> Each customer's money ONLY pays their own invoices</li>
            <li><strong>Records result:</strong> Full payment, Partial payment, or Unpaid</li>
            <li><strong>Tracks unused:</strong> Transactions with no matching customer invoices are marked as UNUSED</li>
          </ol>
        </div>
        ...
        <div className="date-range-picker" style={{ marginTop: '1rem' }}>
          ...
          <div className="input-group">
            ...
            >
              ...
              <option value="bank">🏦 BANK (CRDB)</option>
            </select>
          </div>
        </div>
          ...
          {processedPayments.length > 0 && (
            <>
              {invoicePayments.length > 0 && (
                ...
                >
                  💾 Download Processed Invoices ({invoicePayments.length})
                </button>
              )}
              
              {unusedTransactions.length > 0 && (
                <button 
                  className="button-secondary" 
                  onClick={downloadUnusedCSV}
                  style={{ background: '#f59e0b', color: 'white', border: 'none' }}
                >
                  ⚠️ Download Unused Transactions ({unusedTransactions.length})
                </button>
              )}
            </>
          )}
      ...
      {processedPayments.length > 0 && (
        <div className="card">
          ...
          <div className="stats-grid">
            <div className="stat-card">
              <h3>{invoiceTotals.size}</h3>
              <p>Total Invoices</p>
            </div>
            <div className="stat-card">
              <h3>{paidInvoices}</h3>
              <p>Fully Paid Invoices</p>
            </div>
            <div className="stat-card">
              <h3>{partiallyPaidInvoices}</h3>
            ...
            {unusedTransactions.length > 0 && (
              <div className="stat-card" style={{ background: '#fff3cd' }}>
                <h3>{unusedTransactions.length}</h3>
                <p>Unused Transactions</p>
                <p style={{ fontSize: '0.8rem', marginTop: '0.5rem', color: '#856404' }}>
                  TZS {totalUnusedAmount.toLocaleString()}
                </p>
              </div>
            )}
          </div>
            ...
            <table>
              ...
              <tbody>
                {processedPayments.map((payment, index) => (
                  <tr key={index} style={{ background: payment.isUnused ? '#fff3cd' : 'white' }}>
                    <td>{payment.paymentDate}</td>
                    <td>{payment.customerName}</td>
                    <td>{payment.invoiceNo}</td>
                    <td>
                      {payment.isUnused || payment.invoiceNo === 'UNUSED' ? (
                        <span style={{ color: '#999', fontStyle: 'italic' }}>-</span>
                      ) : (
                        `TZS ${payment.invoiceAmount.toLocaleString()}`
                      )}
                    </td>
                    <td>
                      {payment.isUnused || payment.invoiceNo === 'UNUSED' ? (
                        <span style={{ fontWeight: '600', color: '#f59e0b' }}>
                          TZS {(payment.transactionAmount || payment.amount).toLocaleString()}
                        </span>
                      ) : (
...
//       // 🔥 Count both processed and unused
//       const invoicePaymentsCount = response.data.data.filter(p => p.invoiceNo !== 'UNUSED').length;
//       const unusedCount = response.data.data.filter(p => p.isUnused).length;
...
//   // 🔥 Download only PROCESSED invoice payments (exclude UNUSED)
//   const downloadCSV = () => {
//     if (processedPayments.length === 0) {
//       setError('No processed payments to download');
//       return;
...
//   // 🔥 Download ONLY unused transactions with REAL transaction amounts
//   const downloadUnusedCSV = () => {
...
//       'Transaction Date': payment.paymentDate,
//       'Customer': payment.customerName.toUpperCase(),
//       'Transaction Amount': payment.transactionAmount || payment.amount, // 🔥 Use the REAL transaction amount
//       'Transaction ID': payment.memo || '',
//       'Payment Method': payment.paymentMethod,
//       'Channel': payment.channel || 'N/A',
//       'Status': 'UNUSED - No Matching Invoice',
...
//   // 🔥 FIXED: Only count invoice payments (exclude UNUSED)
//   const invoicePayments = processedPayments.filter(p => !p.isUnused && p.invoiceNo !== 'UNUSED');
//   const unusedTransactions = processedPayments.filter(p => p.isUnused || p.invoiceNo === 'UNUSED');
  
//   // 🔥 FIXED: Only sum numeric amounts (exclude UNUSED)
//   const totalPaidAmount = invoicePayments
//     .filter(p => typeof p.amount === 'number')
//     .reduce((sum, payment) => sum + payment.amount, 0);
  
//   // 🔥 Calculate total unused amount
//   const totalUnusedAmount = unusedTransactions
//     .reduce((sum, payment) => sum + (payment.transactionAmount || payment.amount || 0), 0);
  
//   // 🔥 FIXED: Count unique fully paid invoices using isFullyPaid flag
//   const invoiceTotals = new Map();
//   invoicePayments.forEach(p => {
//     const current = invoiceTotals.get(p.invoiceNo) || { 
//       totalPaid: 0, 
...
//   // 🔥 FIXED: Only sum numeric amounts (exclude UNUSED)
//   const totalPaidAmount = processedPayments
//     .filter(p => typeof p.amount === 'number')
//     .reduce((sum, payment) => sum + payment.amount, 0);
...
//   invoicePayments.forEach(p => {
//     if (!uniqueInvoices.has(p.invoiceNo)) {
//       uniqueInvoices.set(p.invoiceNo, {
...
//   const paidInvoices = Array.from(uniqueInvoices.values()).filter(inv => inv.isFullyPaid).length;
//   const partiallyPaidInvoices = Array.from(uniqueInvoices.values()).filter(inv => !inv.isFullyPaid && inv.totalPaid > 0).length;
//   const unpaidInvoices = Array.from(uniqueInvoices.values()).filter(inv => inv.totalPaid === 0).length;
...
//         complete: (results) => {
//           const parsedInvoices = results.data.map((row, index) => {
//             // Remove commas from amount before parsing (e.g., "12,500.00" -> "12500.00")
//             const amountStr = (row['Amount'] || '0').toString().replace(/,/g, '');
//             const amount = parseFloat(amountStr) || 0;
